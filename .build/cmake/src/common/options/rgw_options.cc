...
std::vector<Option> get_rgw_options() {
  return std::vector<Option>({
    ...
    Option("rgw_disable_s3select", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("disable the s3select operation; RGW will report an error and will return ERR_INVALID_REQUEST.")
    .set_default(false)
    .add_service("rgw"),

    Option("rgw_parquet_buffer_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
    .set_description("the Maximum parquet buffer size, a limit to memory consumption for parquet reading operations.")
    .set_default(16_M)
    .add_service("rgw"),
    ...
    .set_description("Enables the lifecycle maintenance thread. This is required on at least one RGW daemon for each zone.")
    ...
    .set_description("A list of RESTful APIs for RGW to enable")
    ...
    .set_description("set the RGW daemon'ss CPU affinity to a NUMA node (-1 for none)")
    .set_default(-1)
    ...
    Option("rgw_restore_debug_interval", Option::TYPE_INT, Option::LEVEL_DEV)
    .set_description("The number of seconds that simulate one \"day\" in order to debug RGW CloudRestore. Do *not* modify for a production cluster.")
    .set_long_description("For debugging RGW Cloud Restore, the number of seconds that are equivalent to one simulated \"day\". Values less than 1 are ignored and do not change Restore behavior. For example, during debugging if one wanted every 10 minutes to be equivalent to one day, then this would be set to 600, the number of seconds in 10 minutes.")
    .set_default(-1)
    .add_service("rgw"),
    ...
    .set_description("Specify whether S3 authentication uses credentials stored in RADOS")
    .set_default(true)
    .add_service("rgw"),

    Option("rgw_s3_auth_use_keystone", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Specify whether S3 authentication uses Keystone")
    .set_default(false)
    .add_service("rgw"),

    Option("rgw_s3_auth_order", Option::TYPE_STR, Option::LEVEL_ADVANCED)
    .set_description("Authentication strategy order to use for S3")
    .set_long_description("The order of authentication strategies to try for S3 authentication. The value is a comma-separated list of engines, external and local. The default order is to try all configured external engines before attempting local RADOS based authentication")
    .set_default("sts, external, local")
    .add_service("rgw"),

    Option("rgw_s3_auth_disable_signature_url", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Should authentication with presigned URLs be disabled")
    .set_long_description("If enabled, any request that is presigned with either V2 or V4 signature will be denied")
    .set_default(false)
    .add_service("rgw"),
    ...
    .set_description("Space-separated list of LDAP servers in URI format, e.g., \"ldaps://<ldap.your.domain>\".")
    .add_service("rgw"),
    ...
    .set_long_description("Note that multisite replication requires the value admin, so this option must be left at the default in such deployments.")
    .set_default("admin")
    .add_service("rgw"),
    ...
    .set_default(8)
    .add_service("rgw")
    .add_see_also({"rgw_cache_enabled", "rgw_max_control_aio"}),

    Option("rgw_max_control_aio", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("Maximum number of concurrent operations over control objects.")
    .set_long_description("When metadata caching is enabled, a watch operation is sent to each control object on startup, with a corresponding unwatch on shutdown. To accelerate startup/shutdown, allow several concurrent operations to be sent at once.")
    .set_default(8)
    .add_service("rgw")
    .add_see_also({"rgw_num_control_oids"}),
    ...
    Option("rgw_graceful_stop", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Delay the shutdown until all outstanding requests have completed")
    .set_long_description("Wait for up to `rgw_exit_timeout_secs` for all outstanding requests to complete before exiting unconditionally. (new HTTP requests will not be accepted during this time.)")
    .set_default(false)
    .add_service("rgw")
    .add_see_also({"rgw_exit_timeout_secs"}),
    ...
    Option("rgw_asio_assert_yielding", Option::TYPE_BOOL, Option::LEVEL_DEV)
    .set_description("Trigger an assertion failure if an operation would block an asio thread")
    .set_default(false)
    .add_service("rgw"),
    ...
    Option("rgw_account_default_quota_max_objects", Option::TYPE_INT, Option::LEVEL_BASIC)
    .set_description("Account quota max objects")
    .set_long_description("The default quota configuration for total number of objects for a single account. A negative number means 'unlimited'.")
    .set_default(-1)
    .add_service("rgw"),

    Option("rgw_account_default_quota_max_size", Option::TYPE_INT, Option::LEVEL_BASIC)
    .set_description("Account quota max size")
    .set_long_description("The default quota configuration for total size of objects for a single account. A negative number means 'unlimited'.")
    .set_default(-1)
    .add_service("rgw"),
    ...
    .set_long_description("This configurable controls whether RGW enables the website control APIs. RGW can serve static websites if S3 website hostnames are configured.")
    .set_default(false)
    .add_service("rgw"),

    Option("rgw_enable_mdsearch", Option::TYPE_BOOL, Option::LEVEL_BASIC)
    .set_description("Enable elastic metadata search APIs")
    .set_long_description("This configurable controls whether RGW enables the elastic metadata search APIs.")
    .set_default(true)
    .add_service("rgw"),
    ...
    Option("rgw_debug_inject_latency_bi_unlink", Option::TYPE_UINT, Option::LEVEL_DEV)
    .set_description("Latency (in seconds) injected before rgw bucket index unlink op calls to simulate queueing latency and validate behavior of simultaneuous delete requests which target the same object.")
    .set_default(0)
    .add_service("rgw"),

    Option("rgw_reshard_progress_judge_interval", Option::TYPE_UINT, Option::LEVEL_DEV)
    .set_description("interval (in seconds) of judging if bucket reshard failed in block state")
    .set_default(120)
    .add_service("rgw"),

    Option("rgw_reshard_progress_judge_ratio", Option::TYPE_FLOAT, Option::LEVEL_DEV)
    .set_description("ratio of reshard progress judge interval to randomly vary")
    .set_long_description("Add a random delay to rgw_reshard_progress_judge_interval for deciding when to judge the reshard process. The default setting spreads judge time window of [1, 1.5] * rgw_reshard_progress_judge_interval.")
    .set_default(0.5)
    .add_service("rgw")
    .add_see_also({"rgw_reshard_progress_judge_interval"}),

    Option("rgw_reshardlog_threshold", Option::TYPE_UINT, Option::LEVEL_DEV)
    .set_description("threshold for a shard to record log before blocking writes")
    .set_default(30000)
    .add_service({"rgw", "osd"})
    .add_see_also({"rgw_reshard_progress_judge_interval"}),
    ...
    Option("rgw_dynamic_resharding_may_reduce", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Whether dynamic resharding can reduce the number of shards")
    .set_long_description("If true, RGW's dynamic resharding ability is allowed to reduce the number of shards if it appears there are too many.")
    .set_default(true)
    .add_service("rgw")
    .add_see_also({"rgw_dynamic_resharding"}),

    Option("rgw_dynamic_resharding_reduction_wait", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("Number of hours to delay bucket index shard reduction.")
    .set_long_description("In order to avoid resharding buckets with object counts that fluctuate up and down regularly, we implemement a delay between noting a shard reduction might be appropriate and when it's actually done. This allows us to cancel the reshard operation if the number of object significantly increases during this delay. WARNING: Setting this value too low could result in significantly reduced cluster performance.")
    .set_default(120)
    .set_min(0)
    .add_service("rgw")
    .add_see_also({"rgw_dynamic_resharding", "rgw_dynamic_resharding_may_reduce"}),
    ...
    Option("rgw_reshard_debug_interval", Option::TYPE_INT, Option::LEVEL_DEV)
    .set_description("The number of seconds that simulate one \"day\" in order to debug RGW dynamic resharding. Do *not* modify for a production cluster.")
    .set_long_description("For debugging RGW dynamic resharding, the number of seconds that are equivalent to one simulated \"day\". Values less than 1 are ignored and do not change dynamic resharding behavior. For example, during debugging if one wanted every 10 minutes to be equivalent to one day, then this would be set to 600, the number of seconds in 10 minutes.")
    .set_default(-1)
    .add_service("rgw")
    .add_see_also({"rgw_dynamic_resharding", "rgw_reshard_thread_interval", "rgw_dynamic_resharding_reduction_wait"}),
    ...
    .set_long_description("Key used for encrypting/ decrypting role session tokens. This key must consist of 16 hexadecimal characters, which can be generated by the command 'openssl rand -hex 16'. All radosgw instances in a zone should use the same key. In multisite configurations, all zones in a realm should use the same key.")
    .set_default("sts")
    .add_service("rgw"),
    ...
    .set_description("Upper bound on results in listing operations, ListObjects max-keys")
    .set_long_description("This caps the maximum permitted value for listing-like operations in RGW S3. Affects ListObjects(max-keys), ListObjectsVersions(max-keys), ListMultipartUploads(max-uploads), ListParts(max-parts)")
    .set_default(5000)
    ...
    Option("rgw_d4n_l1_datacache_persistent_path", Option::TYPE_STR, Option::LEVEL_ADVANCED)
    .set_description("path used for storing locally cached object data")
    .set_long_description("One cache backend option for D4N is the local SSD, which uses this path to write and read object data. This is the default cache backend chosen by the D4N filter. Only the SSD cache backend uses this path for object data storage since the RedisDriver uses a Redis server instead and there are no additional cache backend implementations available at the moment.")
    .set_default("/tmp/rgw_d4n_datacache/")
    .add_service("rgw"),

    Option("rgw_d4n_l1_datacache_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
    .set_description("maximum size on disk for datacache")
    .set_long_description("The local SSD cache uses this option to configure its size in bytes. This option is not used by the Redis cache backend.")
    .set_default(1_G)
    .add_service("rgw"),

    Option("rgw_d4n_l1_evict_cache_on_start", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("clear the contents of the persistent datacache on start")
    .set_long_description("The local SSD cache uses this option to clear the contents of the path supplied by the rgw_d4n_l1_datacache_persistent_path config option on start. If false, the path's contents will be retained.")
    .set_default(true)
    .add_service("rgw"),

    Option("rgw_d4n_l1_fadvise", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("posix_fadvise() flag for access pattern of cache files")
    .set_long_description("For example, to bypass the page-cache - POSIX_FADV_DONTNEED=4")
    .set_default(4)
    .add_service("rgw"),

    Option("rgw_d4n_l1_write_open_flags", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("cache files write 'man 2 open' 'file status flags' modifiers")
    .set_long_description("For example, to configure synchronized I/O, fcntl-linux.h defines (converted from octal) O_SYNC = 1052672 O_DSYNC = 4096 (0 for no modification of the flags)")
    .set_default(1052672)
    .add_service("rgw"),

    Option("rgw_d4n_libaio_aio_threads", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("specifies the maximum number of worker threads that may be used by libaio")
    .set_long_description("This option is used by the SSD cache backend during initialization to set the maximum number of worker threads libaio may use. It does not apply to the Redis cache backend.")
    .set_default(20)
    .add_service("rgw")
    .add_see_also({"rgw_thread_pool_size"}),

    Option("rgw_d4n_libaio_aio_num", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("specifies the maximum number of simultaneous I/O requests that libaio expects to enqueue")
    .set_long_description("This option is used by the SSD cache backend during initialization to set the maximum number of simultaneous I/O requests that libaio can expect to enqueue. It does not apply to the Redis cache backend.")
    .set_default(64)
    .add_service("rgw")
    .add_see_also({"rgw_thread_pool_size"}),

    Option("rgw_lfuda_sync_frequency", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("LFUDA variables' sync frequency in seconds")
    .set_long_description("By default, the D4N cache uses the Least Frequently Used with Dynamic Aging (LFUDA) cache replacement policy. This class globally stores values that are used by the policy's algorithm. However, strong consistency for these values is not necessary and adds additional overhead to support. As a result, a thread periodically retrieves these global values and posts updates when certain conditions are satisfied. This Redis thread completes this logic in a loop that is called once every interval, with the interval being set by this option.")
    .set_default(60)
    .set_flag(Option::FLAG_STARTUP)
    .add_service("rgw"),

    Option("d4n_writecache_enabled", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("The d4n write cache")
    .set_default(false)
    .set_flag(Option::FLAG_STARTUP)
    .add_service("rgw"),
    ...
    Option("rgw_d4n_address", Option::TYPE_STR, Option::LEVEL_ADVANCED)
    .set_description("address for the D4N Redis connection")
    .set_long_description("The current D4N implementation supports one Redis node which the D4N directory, policy, and overall filter communicate with. This default value is also the address that a Redis server with no additional configuration will use.")
    .set_default("127.0.0.1:6379")
    .set_flag(Option::FLAG_STARTUP)
    .add_service("rgw"),

    Option("rgw_d4n_cache_cleaning_interval", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("This is the interval in seconds for invoking write cache cleaning process")
    .set_default(1000)
    .set_flag(Option::FLAG_STARTUP)
    .add_service("rgw"),
    ...
    Option("rgw_topic_require_publish_policy", Option::TYPE_BOOL, Option::LEVEL_BASIC)
    .set_description("Whether to validate user permissions to publish notifications to topics.")
    .set_long_description("If true, all users (other then the owner of the topic) will need to have a policy to publish notifications to topics. The topic policy can be set by owner via CreateTopic() or SetTopicAttribute(). Following permissions can be granted \"sns:Publish\", \"sns:GetTopicAttributes\", \"sns:SetTopicAttributes\", \"sns:DeleteTopic\" and \"sns:CreateTopic\" via Policy. NOTE that even if set to \"false\" topics will still follow the policies if set on them.")
    .set_default(false)
    .add_service("rgw"),
    ...
    Option("rgw_kafka_connection_idle", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("Time in seconds to delete idle kafka connections")
    .set_long_description("A conection will be considered \"idle\" if no messages are sent to it for more than the time defined. Note that the connection will not be considered idle, even if it is down, as long as there are attempts to send messages to it.")
    .set_default(300)
    .add_service("rgw"),

    Option("rgw_kafka_sleep_timeout", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("Time in milliseconds to sleep while polling for kafka replies")
    .set_long_description("This will be used to prevent busy waiting for the kafka replies As well as for the cases where the broker is down and we try to reconnect. The same values times 3 will be used to sleep if there were no messages sent or received across all kafka connections")
    .set_default(10)
    .add_service("rgw"),

    Option("rgw_kafka_message_timeout", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("This is the maximum time in milliseconds to deliver a message (including retries)")
    .set_long_description("Delivery error occurs when the message timeout is exceeded. Value must be greater than zero, if set to zero, a value of 1 millisecond will be used.")
    .set_default(5000)
    .add_service("rgw"),

    Option("rgw_d4n_l1_datacache_address", Option::TYPE_STR, Option::LEVEL_ADVANCED)
    .set_description("local Redis cache address")
    .set_long_description("This is the address used to configure the Redis cache backend connection. The default value is the same address used by Redis without any additional configuration. The SSD cache does not use this option.")
    .set_default("127.0.0.1:6379")
    .set_flag(Option::FLAG_STARTUP)
    .add_service("rgw"),

    Option("rgw_bucket_logging_obj_roll_time", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("Default time in seconds for the bucket logging object to roll")
    .set_long_description("Object roll time can be provided in the bucket logging configuration. If not provided, this value will be used.")
    .set_default(300)
    .add_service("rgw"),

    Option("rgw_lua_max_runtime_per_state", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("Maximum runtime for each Lua state in milliseconds")
    .set_long_description("Sets the maximum runtime for each Lua state in milliseconds. If exceeded, the script will be terminated. Defaults to 1000 milliseconds (1 second). If set to zero, there is no limit.")
    .set_default(1000)
    .add_service("rgw"),

    Option("rgw_lua_enable", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Enable lua scripting.")
    .set_default(true)
    .set_flag(Option::FLAG_STARTUP)
    .add_service("rgw"),

    Option("rgw_d4n_backend_address", Option::TYPE_STR, Option::LEVEL_ADVANCED)
    .set_description("The backend address used by D4N cache")
    .set_default("127.0.0.1:6379")
    .set_flag(Option::FLAG_STARTUP)
    .add_service("rgw"),

    Option("rgw_bucket_eexist_override", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("CreateBucket on an existing bucket of the same owner returns 409/EEXIST, regardless of region.")
    .set_long_description("Overrides the documented RGW behavior that treats multiple CreateBucket operations by the same owner as idempotent")
    .set_default(false)
    .add_service("rgw"),
