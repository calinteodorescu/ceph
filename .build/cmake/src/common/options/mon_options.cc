...
std::vector<Option> get_mon_options() {
  return std::vector<Option>({
    ...
    Option("mon_down_uptime_grace", Option::TYPE_SECS, Option::LEVEL_ADVANCED)
    .set_description("Period in seconds that the cluster may have a mon down after this (leader) monitor comes up.")
    .set_default(1_min)
    .set_flag(Option::FLAG_RUNTIME)
    .add_service("mon"),
    ...
    Option("mon_nvmeofgw_beacon_grace", Option::TYPE_SECS, Option::LEVEL_ADVANCED)
    .set_description("Period in seconds from last beacon to monitor marking a  NVMeoF gateway as failed")
    .set_default(10)
    .add_service("mon"),

    Option("mon_nvmeofgw_skip_failovers_interval", Option::TYPE_SECS, Option::LEVEL_ADVANCED)
    .set_description("Period in seconds in which no failovers are performed in GW's pool-group this is equal to max GW redeploy interval")
    .set_default(16)
    .add_service("mon"),

    Option("mon_nvmeofgw_set_group_id_retry", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("Retry wait time in microsecond for set group id between the monitor client and gateway")
    .set_long_description("The monitor server determines the gateway's group ID. If the monitor client receives a monitor group ID assignment before the gateway is fully up during initialization, a retry is required.")
    .set_default(1000),

    Option("mon_nvmeofgw_beacons_till_ack", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("Number of beacons from MonClient before NVMeofGwMon sends ack-map to it")
    .set_default(15)
    .add_service("mon"),

    Option("mon_nvmeofgw_delete_grace", Option::TYPE_SECS, Option::LEVEL_ADVANCED)
    .set_description("Issue NVMEOF_GATEWAY_DELETING health warning after this amount of time has elapsed")
    .set_default(15_min)
    .add_service("mon"),
    ...
    Option("mon_cluster_log_level", Option::TYPE_STR, Option::LEVEL_ADVANCED)
    .set_description("Lowest level to include in cluster log file and/or in external log server")
    .set_long_description("Log level to control the cluster log message verbosity for the cluster log file as well as for all external entities.")
    .set_default("debug")
    .set_flag(Option::FLAG_RUNTIME)
    .add_service("mon")
    .add_see_also({"mon_cluster_log_file"}),
    ...
    Option("mon_fsmap_prune_threshold", Option::TYPE_SECS, Option::LEVEL_ADVANCED)
    .set_description("prune fsmap older than this threshold in seconds")
    .set_default(300)
    .set_flag(Option::FLAG_RUNTIME)
    .add_service("mon"),
    ...
    Option("nvmeof_mon_client_disconnect_panic", Option::TYPE_SECS, Option::LEVEL_ADVANCED)
    .set_description("The duration, expressed in seconds, after which the nvmeof gateway should trigger a panic if it loses connection to the monitor")
    .set_default(100)
    .add_service("mon"),

    Option("nvmeof_mon_client_tick_period", Option::TYPE_SECS, Option::LEVEL_ADVANCED)
    .set_description("Period in seconds of nvmeof gateway beacon messages to monitor")
    .set_default(2)
    .add_service("mon"),
