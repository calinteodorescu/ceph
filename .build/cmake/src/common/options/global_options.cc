...
std::vector<Option> get_global_options() {
  return std::vector<Option>({
    ...
    Option("tmp_dir", Option::TYPE_STR, Option::LEVEL_ADVANCED)
    .set_description("path for the 'tmp' directory")
    .set_default("/tmp")
    .set_flag(Option::FLAG_RUNTIME)
    .add_service("common")
    .add_see_also({"admin_socket"}),

    Option("tmp_file_template", Option::TYPE_STR, Option::LEVEL_ADVANCED)
    .set_description("Template for temporary files created by daemons for ceph tell commands")
    .set_long_description("The template file name prefix for temporary files. For example, temporary files may be created by `ceph tell` commands using the --daemon-output-file switch.")
    .set_daemon_default("$tmp_dir/$cluster-$name.XXXXXX")
    .set_flag(Option::FLAG_RUNTIME)
    .add_service({"osd", "mds", "mon"}),
    ...
    Option("breakpad", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("setup breakpad crash handler")
    .set_default(false)
    .set_flag(Option::FLAG_STARTUP),
    ...
    .set_default("C:/Program Files (x86)/ceph/lib/ceph/erasure-code")
    .set_flag(Option::FLAG_STARTUP)
    .add_service({"mon", "osd"}),
    ...
    .set_default("C:/Program Files (x86)/ceph/lib/ceph")
    .set_flag(Option::FLAG_STARTUP)
    .add_service({"mon", "osd"}),
    ...
    Option("qat_compressor_busy_polling", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Set QAT busy bolling to reduce latency at the cost of potentially increasing CPU usage")
    .set_default(false),
    ...
    Option("uadk_wd_sync_ctx_num", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("Set the number of instances in the queue")
    .set_default(2)
    .set_min_max(2, 1024),
    ...
    Option("ms_time_events_min_wait_interval", Option::TYPE_UINT, Option::LEVEL_DEV)
    .set_description("In microseconds, msgr-worker's time_events min wait time for epoll_wait timeout")
    .set_default(1000)
    .set_min_max(0, 60000000),

    Option("ms_client_throttle_retry_time_interval", Option::TYPE_UINT, Option::LEVEL_DEV)
    .set_description("In microseconds, user client, the time interval between the next retry when the throttle get_or_fail.")
    .set_default(5000)
    .set_min_max(1000, 60000000),
    ...
    Option("mon_max_nvmeof_epochs", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("max number of nvmeof gateway maps to store")
    .set_default(500)
    .add_service("mon"),
    ...
    .set_description("Raise a health warning when shallow scrubs are delayed by this percentage of the shallow scrub interval. Note that this must be set at mgr or with global scope. Set this configurable with the command \"ceph config set mgr mon_warn_pg_not_scrubbed_ratio <ratio_value>\".")
    .set_default(0.5)
    .set_min(0.0)
    .add_see_also({"osd_scrub_max_interval"}),

    Option("mon_warn_pg_not_deep_scrubbed_ratio", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
    .set_description("Percentage of the deep scrub interval past the deep scrub interval to warn - Set this configurable with the command \"ceph config set mgr mon_warn_pg_not_deep_scrubbed_ratio <ratio_value>")
    .set_default(0.75)
    .set_min(0.0)
    ...
    Option("mon_debug_no_require_squid", Option::TYPE_BOOL, Option::LEVEL_DEV)
    .set_description("do not set squid feature for new mon clusters")
    .set_default(false)
    .set_flag(Option::FLAG_CLUSTER_CREATE)
    .add_service("mon"),

    Option("mon_debug_no_require_tentacle", Option::TYPE_BOOL, Option::LEVEL_DEV)
    .set_description("do not set tentacle feature for new mon clusters")
    .set_default(false)
    .set_flag(Option::FLAG_CLUSTER_CREATE)
    .add_service("mon"),
    ...
    Option("mon_client_hunt_on_resend", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_default(true),
    ...
    .set_default("plugin=isa technique=reed_sol_van k=2 m=2")
    .set_flag(Option::FLAG_RUNTIME)
    .add_service("mon"),

    Option("osd_pool_default_read_ratio", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("Default read ratio (the percent of read IOs out of all IOs) for a pool.")
    .set_long_description("Default read ratio (the percent of read IOs out of all IOs) for a pool. applicable to replicated pools only. This value is used to improve read balancing when OSDs have different weights.")
    .set_default(70)
    .set_flag(Option::FLAG_RUNTIME)
    .add_service("mon"),
    ...
    Option("osd_pool_default_flag_ec_optimizations", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Control whether to create new erasure coded pools with EC optimizations turned on by default.")
    .set_default(false)
    .add_service("mon"),
    ...
    Option("osd_pg_stat_report_interval_max_seconds", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("How often (in seconds) should PGs stats be collected.")
    .set_default(5),

    Option("osd_pg_stat_report_interval_max_epochs", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("The maximum number of epochs allowed to pass before PG stats are collected.")
    .set_default(500),
    ...
    .set_description("See https://tracker.ceph.com/issues/64002")
    .set_default(false),
    ...
    Option("osd_objectstore_ideal_list_max", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("The max number of results of ObjectStore::collection_list()")
    .set_long_description("This value caps the maximal number of entries a single call to collection_list() can return. The configurable controls this aspect of PG deletion and OSD::clear_temp_objects(). Increasing it trade-offs less agressive chunking (and thus less CPU consumption overall) for higher memory pressure. Please note that in the case of PG deletion the chunking is steered by std::min of the this value and the value of osd_target_transaction_size.")
    .set_default(64)
    ...
    Option("bdev_aio_submit_retry_max", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_default(16),

    Option("bdev_aio_submit_retry_initial_delay_us", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_default(125),
    ...
    .set_description("send discards to the block device")
    .set_default(false)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"bdev_async_discard_threads"}),

    Option("bdev_async_discard_threads", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("number of discard threads used to issue discards to the device")
    .set_default(0)
    .set_min(0)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"bdev_enable_discard", "bdev_async_discard_max_pending"}),

    Option("bdev_async_discard_max_pending", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("maximum number of pending discards")
    .set_long_description("The maximum number of pending async discards that can be queued and not claimed by an async discard thread. Discards will not be issued once the queue is full and blocks will be freed back to the allocator immediately instead. This is useful if you have a device with slow discard performance that can't keep up to a consistently high write workload. 0 means 'unlimited'.")
    .set_default(1000000)
    .set_min(0)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"bdev_async_discard_threads"}),
    ...
    Option("bluefs_wal_envelope_mode", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Enables a faster backend in BlueFS for WAL writes.")
    .set_long_description("In envelope mode BlueFS files do not need to update metadata. When applied to RocksDB WAL files, it reduces by ~50% the amount of fdatasync syscalls. Downgrading from an envelope mode to legacy mode requires `ceph-bluestore-tool --command downgrade-wal-to-v1`.")
    .set_default(true),

    Option("bluefs_allocator", Option::TYPE_STR, Option::LEVEL_DEV)
    .set_default("hybrid")
    .set_enum_allowed({"bitmap", "stupid", "avl", "btree", "hybrid", "hybrid_btree2"}),
    ...
    Option("bluestore_bdev_label_multi", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Keep multiple copies of block device label.")
    .set_long_description("Having multiple labels is only useful in error conditions. The label located at offset 0 has been known to be sometimes overwritten by unknown causes, but without it OSD cannot run.")
    .set_default(true)
    .set_flag(Option::FLAG_CREATE),

    Option("bluestore_bdev_label_require_all", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Require all copies to match.")
    .set_long_description("Under normal conditions, all copies should be the same. Clearing this flag allows to run OSD if at least one of labels could be properly read.")
    .set_default(true)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"bluestore_bdev_label_multi"}),

    Option("bluestore_bdev_label_multi_upgrade", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Let repair upgrade to multi label.")
    .set_long_description("By default single label is preserved. Setting this variable before running fsck-repair upgrades single label into multi label.")
    .set_default(false)
    .set_flag(Option::FLAG_STARTUP),
    ...
    Option("bluestore_recompression_min_gain", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
    .set_description("Required estimated gain for accepting extents for recompressing.")
    .set_long_description("Partial writes over compressed blobs have high cost. New data requires new allocation units, but whole old blob must remain. To combat it BlueStore looks around write position to find blobs that will make it profitable to read and recompress.")
    .set_default(1.2)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"bluestore_compression_max_blob_size"}),
    ...
    Option("bluestore_write_v2", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Use faster write path")
    .set_long_description("Original write path was developed over long time by constantly adding features. The price was layered inefficiencies gained along the way. Rework of write path done from scratch clears it and optimizes for typical cases. Write_v2 is necessary for recompression feature.")
    .set_default(false)
    .set_flag(Option::FLAG_STARTUP),

    Option("bluestore_write_v2_random", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Random selection of write path mode")
    .set_long_description("For testing purposes. If true, value of bluestore_write_v2 is randomly selected on each mount.")
    .set_default(false)
    .set_flag(Option::FLAG_STARTUP)
    .add_see_also({"bluestore_write_v2"}),
    ...
    Option("bluestore_async_db_compaction", Option::TYPE_BOOL, Option::LEVEL_DEV)
    .set_description("Perform DB compaction requests asynchronously")
    .set_long_description("How to perform DB compactions triggered either through async socket or by OSD initialization procedure on start.")
    .set_default(true),
    ...
    Option("bluestore_warn_on_free_fragmentation", Option::TYPE_FLOAT, Option::LEVEL_BASIC)
    .set_description("Level at which disk free fragmentation causes health warning. Set \"1\" to disable. This is same value as admin command \"bluestore allocator score block\".")
    .set_default(0.8)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"bluestore_fragmentation_check_period"}),

    Option("bluestore_fragmentation_check_period", Option::TYPE_UINT, Option::LEVEL_BASIC)
    .set_description("The period to perform bluestore free fragmentation check. Checking fragmentation is usually almost immediate. For highly fragmented storage, it can take several miliseconds. It can cause a stall to a write operation.")
    .set_default(3600)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"bluestore_warn_on_free_fragmentation"}),

    Option("bluestore_slow_ops_warn_lifetime", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("Set the time period during which a BlueStore slow ops warning will be raised when the `bluestore_slow_ops_warn_threshold` is exceeded.  This is not the same as `osd_op_complaint_time`, which is about RADOS ops at the OSD level.")
    .set_default(86400)
    .add_see_also({"bluestore_slow_ops_warn_threshold", "osd_op_complaint_time"}),

    Option("bluestore_slow_ops_warn_threshold", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("Set the minimum number of BluesStore slow ops before raising a health warning state")
    .set_default(1)
    .add_see_also({"bluestore_slow_ops_warn_lifetime", "osd_op_complaint_time"}),
    ...
    Option("bluestore_allocator_lookup_policy", Option::TYPE_STR, Option::LEVEL_ADVANCED)
    .set_description("Determines how to perform the next free extent lookup.")
    .set_long_description("When set to 'hdd_optimized' the allocator searches from the last location found. This may facilitate contiguous disk writes. It may similarly be beneficial for large-IU QLC SSDs to enable firmware coalescing of sub-IU writes. When set to 'ssd-optimized' the allocator will search from the beginning of the device. This may facilitate SSD firmware housekeeping. When set to 'auto' the value will be derived from the detected device type (rotational or non-rotational).")
    .set_default("auto")
    .set_enum_allowed({"hdd_optimized", "ssd_optimized", "auto"}),
    ...
    Option("bluestore_btree2_alloc_weight_factor", Option::TYPE_FLOAT, Option::LEVEL_DEV)
    .set_description("Large continuous extents weight factor")
    .set_default(2.0),
    ...
    Option("rados_replica_read_policy", Option::TYPE_STR, Option::LEVEL_ADVANCED)
    .set_description("read policy for sending read requests to OSD")
    .set_default("default")
    .set_enum_allowed({"default", "balance", "localize"})
    .set_flag(Option::FLAG_RUNTIME),

    Option("rados_replica_read_policy_on_objclass", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("enable read policy for sending read requests to OSD on objclass ops")
    .set_default(false),
    ...
    Option("mgr_enable_op_tracker", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Enable / disable MGR Op Tracker")
    .set_default(true),

    Option("mgr_num_op_tracker_shard", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("The number of shards for holding the ops")
    .set_default(32),

    Option("mgr_op_complaint_time", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
    .set_description("An operation becomes complaint worthy after the specified number of seconds have elapsed.")
    .set_default(30.0),

    Option("mgr_op_log_threshold", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_default(5),

    Option("mgr_op_history_size", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_default(20),

    Option("mgr_op_history_duration", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("The oldest completed operation to track.")
    .set_default(600),

    Option("mgr_op_history_slow_op_size", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("Max number of slow ops to track")
    .set_default(20),

    Option("mgr_op_history_slow_op_threshold", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
    ...
    .set_enum_allowed({"aio", "spdk", "pmem"}),

    Option("bdev_stalled_read_warn_lifetime", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("A configurable duration for stalled read warning to be appeared if number of stalled read occurence pass `bdev_stalled_read_warn_threshold` in `bdev_stalled_read_warn_lifetime` seconds")
    .set_default(86400),

    Option("bdev_stalled_read_warn_threshold", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("A configurable number for stalled read warning to be appeared if number of stalled read occurence pass `bdev_stalled_read_warn_threshold` in `bdev_stalled_read_warn_lifetime` seconds")
    .set_default(1),
    ...
    Option("bluestore_onode_segment_size", Option::TYPE_SIZE, Option::LEVEL_ADVANCED)
    .set_description("Size of segment for onode.")
    .set_long_description("When object size grows too large BlueStore splits allocation metadata into smaller RocksDB keys (shards). When multiple blobs overlap each other some of them might belong to more than one shard. The encoding for such case is inefficient (spanning blobs). Segmentation of data prevents blobs from crossing specific separation lines, preventing spanning blobs altogether. The smaller values give better split on onode shards. The larger values minimize space loss for padding in compression. Recommended values 256K, 512K, 1024K. Value 0 disables segmentation. Actual segment size cannot be smaller than \"compression_max_blob_size\" pool option, if set.")
    .set_default(0)
    .add_see_also({"bluestore_extent_map_shard_max_size", "bluestore_extent_map_shard_target_size", "bluestore_debug_onode_segmentation_random"}),

    Option("bluestore_debug_onode_segmentation_random", Option::TYPE_BOOL, Option::LEVEL_DEV)
    .set_description("Random selection of onode segmentation")
    .set_long_description("For testing purposes. On each mount 50% roll decides whether to use bluestore_onode_segment_size or set it to 0 (disable).")
    .set_default(false)
    .set_flag(Option::FLAG_STARTUP)
    .add_see_also({"bluestore_onode_segment_size"}),
    ...
    Option("objectstore_debug_throw_on_failed_txc", Option::TYPE_BOOL, Option::LEVEL_DEV)
    .set_description("Enables exception throwing instead of process abort on transaction submission error.")
    .set_default(false),

    Option("ceph_assert_supresssions", Option::TYPE_STR, Option::LEVEL_DEV)
    .set_description("Suppress specific ceph_assert instances to let the execution continue with unknown, potentially DESTRUCTIVE consequences."),

    Option("ec_extent_cache_size", Option::TYPE_UINT, Option::LEVEL_ADVANCED)
    .set_description("Size of per-shard extent cache")
    .set_default(10485760)
    .add_service("osd"),

    Option("ec_pdw_write_mode", Option::TYPE_UINT, Option::LEVEL_DEV)
    .set_description("When EC writes should generate PDWs (development only) 0=optimal 1=never 2=when possible")
    .set_default(0),
