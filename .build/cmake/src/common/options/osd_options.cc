...
std::vector<Option> get_osd_options() {
  return std::vector<Option>({
    ...
    .add_see_also({"osd_mclock_override_recovery_settings"}),
    ...
    Option("osd_recovery_sleep_degraded", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
    .set_description("Time in seconds to sleep before next recovery or backfill op when PGs are degraded. This setting overrides _ssd, _hdd, and _hybrid if non-zero.")
    .set_default(0.0)
    .set_flag(Option::FLAG_RUNTIME),

    Option("osd_recovery_sleep_degraded_hdd", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
    .set_description("Time in seconds to sleep before next recovery or backfill op for HDDs when PGs is degraded.")
    .set_default(0.1)
    .set_flag(Option::FLAG_RUNTIME),

    Option("osd_recovery_sleep_degraded_ssd", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
    .set_description("Time in seconds to sleep before next recovery or backfill op for SSDs when PGs are degraded.")
    .set_default(0.0)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"osd_recovery_sleep_degraded"}),

    Option("osd_recovery_sleep_degraded_hybrid", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
    .set_description("Time in seconds to sleep before next recovery or backfill op when PGs are degraded and data is on HDD and journal is on SSD")
    .set_default(0.025)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"osd_recovery_sleep_degraded"}),
    ...
    .set_description("The desired interval between scrubs of a specific PG. Note that this option must be set at ``global`` scope, or for both ``mgr`` and``osd``.")
    .set_default(1_day)
    .add_see_also({"osd_scrub_max_interval"}),

    Option("osd_scrub_max_interval", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
    .set_description("Scrub each PG no less often than this interval. Note that this option must be set at ``global`` scope, or for both ``mgr`` and``osd``.")
    .set_default(7_day)
    ...
    .set_long_description("This prevents a scrub 'stampede' by randomly varying the scrub intervals so that they are uniformly distributed over time.")
    .set_default(0.5)
    .add_see_also({"osd_scrub_min_interval"}),
    ...
    .set_description("Duration (in seconds) of delay injected between chunks when scrubbing")
    .set_default(0.0)
    .set_flag(Option::FLAG_RUNTIME),

    Option("osd_scrub_extended_sleep", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
    .set_description("Duration (in seconds) of delay injected between chunks when scrubbing out of scrubbing hours")
    .set_default(0.0)
    ...
    .set_description("Deep scrub each PG (i.e., verify data checksums) at least this often. Note that this option must be set at ``global`` scope, or for both ``mgr`` and``osd``.")
    .set_default(7_day),

    Option("osd_deep_scrub_interval_cv", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
    .set_description("determining the amount of variation in the deep scrub interval")
    .set_long_description("deep scrub intervals are varied by a random amount to prevent stampedes. This parameter determines the amount of variation. Technically ``osd_deep_scrub_interval_cv`` is the coefficient of variation for the deep scrub interval.")
    .set_default(0.2)
    .set_min_max(0.0, 0.4),

    Option("osd_deep_scrub_randomize_ratio", Option::TYPE_FLOAT, Option::LEVEL_ADVANCED)
    .set_description("deprecated. Has no effect.")
    .set_default(0.15),
    ...
    Option("osd_scrub_retry_delay", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("Period (in seconds) before retrying a PG that has failed a prior scrub.")
    .set_long_description("Minimum delay after a failed attempt to scrub a PG. The delay is either applied to one of the scheduled scrubs for the PG (the next shallow scrub or the next deep scrub), or to both. This is a default value, used when the cause of the delay does not have an associated configuration option. See the 'see also' for the configuration options for some delay reasons that have their own configuration.")
    .set_default(30)
    .set_min(1)
    .add_see_also({"osd_scrub_retry_pg_state", "osd_scrub_retry_after_noscrub", "osd_scrub_retry_new_interval", "osd_scrub_retry_trimming"}),

    Option("osd_scrub_retry_after_noscrub", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("Period (in seconds) before retrying to scrub a PG at a specific level after detecting a no-scrub or no-deep-scrub flag")
    .set_long_description("Minimum delay after a failed attempt to scrub a PG at a level (shallow or deep) that is disabled by cluster or pool no-scrub or no-deep-scrub flags.")
    .set_default(60)
    .set_min(1)
    .add_see_also({"osd_scrub_retry_delay"}),

    Option("osd_scrub_retry_pg_state", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("Period (in seconds) before retrying to scrub a previously inactive/not-clean PG")
    .set_long_description("Minimum delay after a failed attempt to scrub a PG that is not active and clean.")
    .set_default(60)
    .set_min(1)
    .add_see_also({"osd_scrub_retry_delay"}),

    Option("osd_scrub_retry_trimming", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("Period (in seconds) before retrying to scrub a previously snap-trimming PG")
    .set_long_description("Minimum delay after a failed attempt to scrub a PG that was performing snap trimming and not available for scrubbing.")
    .set_default(10)
    .set_min(1)
    .add_see_also({"osd_scrub_retry_delay"}),

    Option("osd_scrub_retry_new_interval", Option::TYPE_INT, Option::LEVEL_ADVANCED)
    .set_description("Period (in seconds) before retrying a scrub aborted on a new interval")
    .set_long_description("Minimum delay before retrying, after a scrub was aborted as the PG interval changed.")
    .set_default(10)
    .set_min(1)
    .add_see_also({"osd_scrub_retry_delay"}),

    Option("osd_scrub_disable_reservation_queuing", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_description("Disable queuing of scrub reservations")
    .set_long_description("When set - scrub replica reservations are responded to immediately, with either success or failure (the pre-Squid version behaviour). This configuration option is introduced to support mixed-version clusters and debugging, and will be removed in the next release.")
    .set_default(false),
    ...
    .set_default("cephfs hello journal lock log numops otp rbd refcount rgw rgw_gc timeindex user version cas cmpomap queue 2pc_queue fifo sem_set"),
    ...
    .set_long_description("This option specifies the high threshold IOPS capacity for an OSD below which the OSD bench results can be considered for QoS calculations. Only considered when osd_op_queue = mclock_scheduler")
    .set_default(500.0)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"osd_mclock_max_capacity_iops_hdd"}),

    Option("osd_mclock_iops_capacity_low_threshold_hdd", Option::TYPE_FLOAT, Option::LEVEL_BASIC)
    .set_description("The threshold IOPs capacity (at 4KiB block size) below which to ignore the OSD bench results for an OSD (for rotational media)")
    .set_long_description("This option specifies the low threshold IOPS capacity of an OSD above which the OSD bench results can be considered for QoS calculations. Only considered when osd_op_queue = mclock_scheduler")
    .set_default(50.0)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"osd_mclock_max_capacity_iops_hdd"}),
    ...
    Option("osd_mclock_iops_capacity_low_threshold_ssd", Option::TYPE_FLOAT, Option::LEVEL_BASIC)
    .set_description("The threshold IOPs capacity (at 4KiB block size) below which to ignore the OSD bench results for an OSD (for solid state media)")
    .set_long_description("This option specifies the low threshold IOPS capacity for an OSD above which the OSD bench results can be considered for QoS calculations. Only considered when osd_op_queue = mclock_scheduler")
    .set_default(1000.0)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"osd_mclock_max_capacity_iops_ssd"}),
    ...
    Option("osd_ec_partial_reads", Option::TYPE_BOOL, Option::LEVEL_ADVANCED)
    .set_default(true),
    ...
    .set_default(0)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"osd_recovery_max_active_hdd", "osd_recovery_max_active_ssd", "osd_mclock_override_recovery_settings"}),
    ...
    .set_default(3)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"osd_recovery_max_active", "osd_recovery_max_active_ssd", "osd_mclock_override_recovery_settings"}),
    ...
    .set_default(10)
    .set_flag(Option::FLAG_RUNTIME)
    .add_see_also({"osd_recovery_max_active", "osd_recovery_max_active_hdd", "osd_mclock_override_recovery_settings"}),
